...
- Lightbox 大图查看：所有展示模式（single/diptych/wall/report/anthology）点击图片弹出 Lightbox 大图，支持左右切换、键盘导航（Esc 关闭、← → 切换）、点击遮罩关闭；双联画模式同时展示一对图片，移动端自动上下堆叠；底部展示作品阐述（Markdown 渲染）
...
- 新建作品集自动创建默认媒体区块和阐述区块；所有类型均可添加/删除媒体区块，非 report 类型限制一个媒体区块
