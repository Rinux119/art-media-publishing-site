...
- 第5の表示モード「アンソロジー（anthology）」：コレクションのトップページは Grid グリッドで複数のアンソロジー入口を表示（各メディアブロックが1つのアンソロジーに対応し、ブロックの最初の画像がサムネイルになります）；入口をクリックするとそのアンソロジーのサムネイルページに移動；各アンソロジーは個別にタイトルを設定でき、タイトルはサムネイルの下に表示されます
