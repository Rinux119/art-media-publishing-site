...
- Lightbox 表示：すべての表示モード（single/diptych/wall/report/anthology）で画像クリック時に Lightbox オーバーレイで拡大表示（新ページ遷移なし）；左右ナビゲーション、キーボード操作（Esc で閉じる、← → で切替）、オーバーレイクリックで閉じる；diptych モードではペア画像を並べて表示（モバイルでは縦積み）；下部に作品解説を Markdown レンダリング
