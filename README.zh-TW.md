...
這是一個基於 Node.js、Express、SQLite 和 EJS 的作品展示與媒體發佈平台。支援圖片、影片、文字等多種形式，包含完整的後台發佈工作流、圖片變體生成和影片轉碼能力。
...
於是我透過 AI 的協助，構建了這個系統。起初這只是根據我個人的使用和展示習慣進行設計的東西，但我現在將其開源，希望各位同樣苦於演算法和學術壟斷的個人創作者都能擁有屬於自己的「白盒子」，解構「藝術作品必須依附於特定機構才能呈現」的敘事。

由於我本人並非科班出身的軟體工程師，僅具備基本的伺服器維運經驗，所以我可能沒有辦法對當前系統進行進一步的優化或擴展。但我非常歡迎各位對此有需求或共鳴的朋友 fork 或使用這個專案，以滿足自己的需求。
...
- Lightbox 大圖查看：所有展示模式（single/diptych/wall/report/anthology/archiving）點擊圖片彈出 Lightbox 大圖，不再跳轉新頁面；支援左右切換、鍵盤導航（Esc 關閉、← → 切換）、點擊遮罩關閉；雙聯畫模式同時展示一對圖片，行動端自動上下堆疊；底部展示作品闡述（Markdown 渲染）
...
- **影片轉碼**：透過 FFmpeg 自動轉碼為 H.264/AAC MP4 格式，解析度壓縮至 1080p，支援漸進式播放
...
- **安全防護**：CSRF 防護、CSP/HSTS 安全頭、IP 暴力破解鎖定、自訂異常類別
- **存取日誌**：公開頁面 IP/路徑/時間戳自動記錄，超限自動清理
- **Session 持久化**：Session 儲存於 SQLite，服務重啟後登入態不遺失
- **優雅關閉**：連線追蹤、健康/就緒檢查、404/500 兜底
- **自動化測試**：91 個整合測試 + 11 個單元測試，覆蓋核心工作流、安全防護與邊界條件，包含 anthology / archiving 二級路由與區塊級 `media_format` 渲染
...
- 設定 SSL/TLS 憑證，啟用 HTTPS（可使用 [Let's Encrypt](https://letsencrypt.org/) 免費取得）
- 使用行程管理器（PM2 / systemd），確保異常退出後自動重啟
...
以下為 HTTP 範例，生產環境建議在 Nginx 層設定 HTTPS（可借助 `certbot` 自動管理憑證）。
