...
const EXCLUDED_IPS = new Set(
    (process.env.EXCLUDED_IPS || '').split(',').map((ip) => ip.trim()).filter(Boolean)
);
...
const createVisitLogger = ({ db }) => {
    ...
    const getRecentVisits = (limit = 10) => {
        ...
        ).all(limit).filter((v) => !EXCLUDED_IPS.has(v.ip));
    };
    ...
    const getVisitStats = () => {
        const excluded = EXCLUDED_IPS.size > 0;
        const placeholders = excluded ? ` AND ip NOT IN (${Array(EXCLUDED_IPS.size).fill('?').join(',')})` : '';
        const total = db.prepare(`SELECT COUNT(*) AS cnt FROM visit_logs WHERE 1=1${placeholders}`).get(...EXCLUDED_IPS);
        const uniqueIps = db.prepare(`SELECT COUNT(DISTINCT ip) AS cnt FROM visit_logs WHERE 1=1${placeholders}`).get(...EXCLUDED_IPS);
        return {
