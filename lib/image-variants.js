...
const createImageVariantService = ({
    ...
    const ensureMediaLibraryImageVariants = async (filename, { priority = 'low', onStep = null } = {}) => {
        if (!filename || !isImageFile(filename) || typeof getMediaLibraryDir !== 'function') return;

        const originalPath = path.join(getMediaLibraryDir('original'), filename);
        const sourcePath = fs.existsSync(originalPath) ? originalPath : null;
        if (!sourcePath) return;

        const generate = async (size) => {
            const targetPath = path.join(getMediaLibraryDir(size), filename);
            const lockKey = `media_library::${size}::${filename}`;
            const ok = await withGenerationLock(lockKey, async () => generateImageVariant({ sourcePath, targetPath, size, priority }));
            if (typeof onStep === 'function') {
                try { onStep({ kind: 'variant', size, ok }); } catch (_) {}
            }
